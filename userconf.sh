...
then
    printf "\n\n"
    tput bold
    printf "\e[31mERROR\e[39m: You must set a unique PASSWORD (not 'bees') first! e.g. run with:\n"
...
## Configure user with a different USERID if requested.
  ...
  then
    ...
    cp -r /home/bees /home/$USER
    ## RENAME the user
    usermod -l $USER -d /home/$USER bees
    groupmod -n $USER bees
    usermod -a -G staff $USER
...
if [ "$GROUPID" -ne 1000 ]
## Configure the primary GID (whether bees or $USER) with a different GROUPID if requested.
  then
